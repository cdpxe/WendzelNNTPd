...
\textbf{Please Note:} \textit{If you wish to compile WendzelNNTPd WITH optional TLS support}, then run \textbf{TLS=YES ./configure}. The default TLS library is \textbf{GnuTLS}, if you want \textbf{OpenSSL} instead, run \textbf{TLS=YES GNUTLS=NO ./configure} to switch the TLS library.
